...
execute unless score @s tse_modelData matches 1.. if score @s tse_enchantCheck matches 1 store success score @s tse_enchantCheck run data get entity @s Inventory[{Slot: -106b}].tag.pages
execute if score @s tse_enchantCheck matches 1 run item modify entity @s weapon.mainhand tser_main:append_lore/0
...
execute if score @s tse_enchantCheck matches 0 run tellraw @s ["",{"text":"[Offhand Lore] ","color":"green"},{"text":"Error: Could not add to the lore. Please use book with JSON text and a non-custom item."}]